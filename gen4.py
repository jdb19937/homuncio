...
SYSTEM_PROMPT = (
    ...
    "four portrait prompts, each labeled with the quadrant it belongs to: "
    "TOP-LEFT, TOP-RIGHT, BOTTOM-LEFT, BOTTOM-RIGHT. Generate a SINGLE square "
    "image arranged as a 2x2 grid of four portraits, placing each portrait in "
    "the quadrant named by its label. Each quadrant must contain exactly one of "
    "the four portraits and must follow its own prompt's framing and "
    "subject-description rules independently. The four quadrants depict four "
    "different people. Use consistent photographic style and lighting across "
    "all four so the grid reads as a coherent set. Do not add borders, gutters, "
    "labels, captions, text, or decoration — just the four portraits tiled "
    "edge-to-edge."
)
...
def main():
    ...
    ap.add_argument('-s', type=int, default=None,
                    help='base seed; prompts use s, s+1, s+2, s+3 (random if omitted)')
    ...
    for q, s, p in zip(quadrants, seeds, prompts):
        print(f'--- {q} (seed={s}) ---', file=sys.stderr)
    ...
    user_msg = '\n\n'.join(
        f'=== {q} QUADRANT ===\n{p}' for q, p in zip(quadrants, prompts)
    )
