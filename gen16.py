...
# 4x4 grid. Cell label is (row, col), 1-indexed.
# row 1 = top, row 4 = bottom. col 1 = left, col 4 = right.
CELLS = [(r, c) for r in range(1, 5) for c in range(1, 5)]

SYSTEM_PROMPT = (
    "You are an image generator. The user will send a single message containing "
    "sixteen portrait prompts, each labeled with its cell in a 4x4 grid. Cell "
    "labels use the form (row,col) with row 1 at the top and row 4 at the "
    "bottom, and col 1 at the left and col 4 at the right — so (1,1) is the "
    "top-left corner and (4,4) is the bottom-right corner. Generate a SINGLE "
    "square image arranged as a 4x4 grid of sixteen portraits, placing each "
    "portrait in the cell named by its label. Each cell must contain exactly "
    "one of the sixteen portraits and must follow its own prompt's framing and "
    "subject-description rules independently. The sixteen cells depict sixteen "
    "different people. Use consistent photographic style and lighting across "
    "all sixteen so the grid reads as a coherent set. Do not add borders, "
    "gutters, labels, captions, text, or decoration — just the sixteen "
    "portraits tiled edge-to-edge."
)
...
def main():
    ...
    ap.add_argument('-s', type=int, default=None,
                    help='base seed; prompts use s, s+1, ..., s+15 (random if omitted)')
    ...
    seeds = [base_seed + i for i in range(16)]
    prompts = [run_promptum(promptum, s, extra) for s in seeds]

    for (r, c), s, p in zip(CELLS, seeds, prompts):
        print(f'--- ({r},{c}) (seed={s}) ---', file=sys.stderr)
    ...
    user_msg = '\n\n'.join(
        f'=== CELL ({r},{c}) ===\n{p}' for (r, c), p in zip(CELLS, prompts)
    )
